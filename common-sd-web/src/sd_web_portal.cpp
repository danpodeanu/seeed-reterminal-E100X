...
void sendPageHeader(const String& title, const String& breadcrumbHtml) {
  ...
  html += F(
      "</title>"
      "<style>"
      "body{font-family:system-ui,sans-serif;margin:0;padding:0;background:#f4f4f4;color:#222}"
      "header{background:#111;color:#fff;padding:1rem;position:sticky;top:0}"
      "header h1{margin:0;font-size:1.1rem}"
      "header .crumb{font-size:.9rem;color:#ccc;margin-top:.4rem;word-break:break-all}"
      "header .crumb a{color:#8cf;text-decoration:none}"
      ...
      "section h2{margin:.2rem 0 1rem 0;font-size:1rem;color:#555}"
      "table{width:100%;border-collapse:collapse;font-size:.95rem}"
      "th,td{text-align:left;padding:.4rem .3rem;border-bottom:1px solid #eee;vertical-align:middle}"
      "th{color:#666;font-weight:600}"
      "td.size,td.mtime,th.size,th.mtime{white-space:nowrap;color:#666;font-variant-numeric:tabular-nums}"
      "td.actions{text-align:right;white-space:nowrap}"
      "a.name{color:#036;text-decoration:none;font-weight:500}"
      "a.name:hover{text-decoration:underline}"
      ".dir a.name::before{content:\"\\1F4C1 \";}"
      ".file a.name::before{content:\"\\1F4C4 \";}"
      "button,.btn{background:#036;color:#fff;border:0;padding:.35rem .6rem;border-radius:4px;cursor:pointer;font-size:.85rem;text-decoration:none;display:inline-block}"
      "button.danger{background:#a22}"
      "form.inline{display:inline}"
      "form.row{display:flex;gap:.5rem;margin-top:.5rem}"
      "form.row input[type=text]{flex:1;padding:.4rem;border:1px solid #ccc;border-radius:4px;font-size:.95rem}"
      ...
      ".flash{background:#efe;border:1px solid #cec;color:#252;padding:.6rem;border-radius:6px;margin-bottom:1rem}"
