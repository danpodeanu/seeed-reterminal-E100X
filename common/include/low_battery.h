...
// populates sensors::Readings::{batteryVoltage, batteryPct, chargerValid,
// externalPower}. This module turns that into a single yes/no verdict.
//
// "Battery sensor present" is inferred from chargerValid: the SY6974B
// PMIC responds on I2C on newer E1001/E1002/E1003/E1004 revisions and
...
// false-positive on a bare-USB power path.
//
...
//   - the charger IC didn't ACK (older E1001/E1002 boards, no reliable
//     "battery is here" signal)
...
inline bool shouldWarn(bool enabled, bool chargerValid, bool externalPower,
                       int batteryPct, int thresholdPct = kThresholdPct) {
  if (!enabled) return false;
  if (!chargerValid) return false;
